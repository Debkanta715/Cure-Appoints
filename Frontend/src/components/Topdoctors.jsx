...
const Topdoctors = () => {
  const navigate = useNavigate()  // by this we change the path
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10 max-w-6xl mx-auto'>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {doctors.slice(0,10).map((item , idx)=>( // by this function we insert the value which is present in the assets
      ...
      <div className='w-full flex justify-center'>
        <button onClick={()=>{navigate('/doctors'); window.scrollTo(0,0)}} className='mt-6 px-6 py-2 bg-primary text-white rounded-full hover:opacity-90'>more</button>
      </div>
    </div>
  )
}
